...
@WebAppConfiguration
@EnableJpaRepositories({ "de.maltorpro.shop.service.test.support",
...
public class ApplicaitonTest {
    ...
    public void test1SaveReview() throws Exception {
        ...
        this.mockMvc
                .perform(post("/review").contentType(
                        org.springframework.http.MediaType.APPLICATION_JSON)
                        .content(TestUtils.asJsonString(review)))
                .andDo(print()).andExpect(status().isOk())
                .andExpect(jsonPath("$.author", equalTo("Max Mustermann2")))
                ...
                .andExpect(jsonPath("$.product.longDescription",
                        equalTo("product2 long description")))
                .andDo(document("review-save", preprocessRequest(prettyPrint()),
                        ...
                        requestFields(
                                ...
                                fieldWithPath("reviewUuid").description(
                                        FieldDescription.REVIEW_UUID_DESCRIPTION),
                                fieldWithPath("author").description(
                                        FieldDescription.REVIEW_AUTHOR_DESCRIPTION),
                                fieldWithPath("rating").description(
                                        FieldDescription.REVIEW_RATING_DESCRIPTION),
                                fieldWithPath("reviewText").description(
                                        FieldDescription.REVIEW_REVIEW_TEXT),
                                subsectionWithPath("product").description(
                                        FieldDescription.REVIEWP_RODUCT_DESCRIPTION))));
        reviewCounter++;
    }
    ...
    public void test2GetReviewsForProduct() throws Exception {
        ...
        this.mockMvc.perform(get("/reviews/product/{uuid}/{page}/{size}",
                product1.getProductUuid(), 0, 4).accept(
                        org.springframework.http.MediaType.APPLICATION_JSON))
                .andDo(print()).andExpect(status().isOk())
                .andExpect(jsonPath("$.content[*]", hasSize(2)))
                .andExpect(jsonPath("$.content[0].author",
                        equalTo("Max Mustermann")))
                .andExpect(jsonPath("$.content[0].reviewText",
                        equalTo("Nice product")))
                .andExpect(jsonPath("$.content[0].rating", equalTo(5)))
                .andExpect(jsonPath("$.content[0].product.name",
                        equalTo("Product1")))
                .andExpect(jsonPath("$.content[0].product.shortDescription",
                        equalTo("product1 short description")))
                .andExpect(jsonPath("$.content[0].product.longDescription",
                        equalTo("product1 long description")))
                .andDo(document("reviews-get", preprocessRequest(prettyPrint()),
                        preprocessResponse(prettyPrint()),
                        ...
                        responseFields(attributes(key("title").value(
                                ...
                                fieldWithPath("content[].updateDate")
                                        .description(
                                                FieldDescription.UPDATE_DATE_DESCRIPTION),
                                fieldWithPath("content[].reviewId").description(
                                        FieldDescription.ID_DESCRIPTION),
                                ...
                                fieldWithPath("content[].author").description(
                                        FieldDescription.REVIEW_AUTHOR_DESCRIPTION),
                                fieldWithPath("content[].rating").description(
                                        FieldDescription.REVIEW_RATING_DESCRIPTION),
                                fieldWithPath("content[].reviewText")
                                        .description(
                                                FieldDescription.REVIEW_REVIEW_TEXT),
    ...
    public void test3UpdateReview() throws Exception {
        ...
        this.mockMvc
                ...
                .andDo(print()).andExpect(status().isOk())
                .andExpect(
                        jsonPath("$.author", equalTo("Max Mustermann updated")))
                .andExpect(jsonPath("$.reviewText",
                        equalTo("Nice product updated")))
                .andExpect(jsonPath("$.rating", equalTo(4)))
                .andExpect(jsonPath("$.product.name", equalTo("Product1")))
                .andExpect(jsonPath("$.product.shortDescription",
                        equalTo("product1 short description")))
                .andExpect(jsonPath("$.product.longDescription",
                        equalTo("product1 long description")));
    ...
    public void test4DeleteReview() throws Exception {
        ...
        assertEquals("Check the review count before delete.", reviewCounter,
        ...
        this.mockMvc.perform(delete("/review/{uuid}", review.getReviewUuid())
        ...
        reviewCounter--;
        assertEquals("Check the review count after delete.", reviewCounter,
                reviewRepository.count());
    }
}
